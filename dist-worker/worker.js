...
async function getCache(c, key) {
  if (c.env && c.env.patungan) {
    try {
      const cached2 = await c.env.patungan.get(key, "json");
...
async function setCache(c, key, data) {
  if (c.env && c.env.patungan) {
    try {
...
api.get("/cors-proxy", async (c) => {
  ...
  try {
    ...
    if (targetUrl.includes(".m3u8") || contentType && contentType.includes("mpegurl")) {
      ...
      text = text.replace(/^[ \t]*(?!#|http)[ \t]*(.+)[ \t]*$/gm, (match2, path) => {
      ...
      text = text.replace(/URI="([^"]+)"/g, (match2, path) => {
...
app.use("/assets/*", module({ manifest }));
app.use("/*", module({ manifest }));
app.get("*", module({ path: "index.html", manifest }));
